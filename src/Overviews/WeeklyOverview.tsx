...
export function getMondayFromSelectedDay(selectedDay: Date): Date {
...
class WeeklyOverview extends React.Component<Props, State> {
  private barData: {
    value: number;
    date: number;
  }[][] = [];
  ...
  shouldComponentUpdate(nextProps: Props, nextState: State) {
    if (nextProps.selectedMonday !== this.props.selectedMonday) {
      return true;
    }
    if (
  ...
  getTrackingDataToDisplay = (trackingType: TrackingTypesEnum) => {
    ...
    const firstDay = this.props.selectedMonday;
    for (var i = 0; i < 7; i++) {
      ...
      case GraphTypeEnum.doubleLine:
        return (
          <DoubleLineChart
            ...
            overviewType={OverviewsEnum.weekly}
          />
        );
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <TrackingTypePicker
          pickerDataSource={trackingTypesActiveOrdered(OverviewsEnum.weekly)}
          overviewType={OverviewsEnum.weekly}
...
const mapStateToProps = (state: any): StateProps => {
  return {
    ...
    selectedTrackingType: state.overviews.weeklySelectedTrackingType,
